...
class RequestContract : ActivityResultContract<Request, Request>() {
    override fun createIntent(context: Context, input: Request): Intent {
        ...
        return when (input) {
            ...
            is Request.UpdateHealing -> Intent(context, CreateNewActivity::class.java).apply { data = uri }
            is Request.UpdatePayment -> Intent(context, CreateNewActivity::class.java).apply { data = uri }
            is Request.UpdatePatient -> Intent(context, NewPatientActivity::class.java).apply { data = uri }
