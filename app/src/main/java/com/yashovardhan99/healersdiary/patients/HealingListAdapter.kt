...
class HealingListAdapter(
    private val onRequestEdit: (HealingParent.Healing) -> Unit,
    private val onRequestDelete: (HealingParent.Healing) -> Unit
) : PagingDataAdapter
<HealingParent, HealingListAdapter.HealingParentViewHolder>(HealingDiffUtils()) {
    ...
    sealed class HealingParentViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
        abstract fun bind(
            healing: HealingParent?,
            onRequestEdit: (HealingParent.Healing) -> Unit,
            onRequestDelete: (HealingParent.Healing) -> Unit
        )

        class HealingViewHolder(val binding: ItemCardHealingBinding) :
            HealingParentViewHolder(binding.root) {
            override fun bind(
                healing: HealingParent?,
                onRequestEdit: (HealingParent.Healing) -> Unit,
                onRequestDelete: (HealingParent.Healing) -> Unit
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HealingParentViewHolder {
        return when (viewType) {
            VIEW_TYPE_HEALING -> HealingParentViewHolder.HealingViewHolder(
                ItemCardHealingBinding.inflate(
                    LayoutInflater.from(parent.context),
                    parent,
                    false
                )
            )
            VIEW_TYPE_SEPARATOR -> HealingParentViewHolder.SeparatorViewHolder(
                ActivitySeparatorBinding.inflate(LayoutInflater.from(parent.context), parent, false)
            )
...
class HealingDiffUtils : DiffUtil.ItemCallback<HealingParent>() {
    override fun areItemsTheSame(oldItem: HealingParent, newItem: HealingParent): Boolean {
        return if (oldItem is HealingParent.Healing && newItem is HealingParent.Healing)
            oldItem.id == newItem.id
        else if (oldItem is HealingParent.HealingSeparator &&
            newItem is HealingParent.HealingSeparator
